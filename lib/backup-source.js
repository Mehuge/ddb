    ...
    try {
      ...
      if (stored && this.lastBackup) {
        ...
        if (last) {
          ...
          stored = false;
          uploaded = '=';         // hash already on server
        }
      }
      stats.files ++;
      ...
      memtrack();
      await this._log(instance, 'F', fn.substr(this.src.length+1), fstat, hash, variant, modified, uploaded);
      if (stored) {
        stats.backedUp.files ++;
        stats.backedUp.bytes += fstat.size;
        if (!this.verbose && this.terse) console.log(`${modified}${uploaded}`, fn);
      }
    } catch(e) {
      stats.skipped ++;
