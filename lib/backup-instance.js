...
class BackupInstance {
  constructor({ target, setname, userid }) {
    this.target = target;
  ...
  getBackupLog() {
    return new BackupLog({ root: this.target.getPath(), userid: this.userid, setname: this.setname });
  }

  async createNewInstance({ comment }) {
    this._log = this.getBackupLog();
    await this._log.create('running', comment);
  }
  ...
  async verify({ when, log, verbose, compare, compareWith }) {
    ...
    let lines = [];
    try {
      lines = await this.log().getLinesFromLog(when);
    } catch(e) {
      throw e;
    }
    function LOG(s) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      case 'F':
        try {
          ...
          if (e.code == 'ECORRUPT') {
            LOG(`CORRUPT ${entry.hash} ${entry.variant} ${entry.size} ${entry.path}`);
          } else if (e.code == 'ENOCOMPARE') {
  ...
  async cat({ when }, args) {
    ...
    args.forEach(file => {
      const entry = lines.find(l => l.type == 'F' && l.path == file);
      if (entry.type == 'F') {
        this.target.fs().cat(entry);
        return;
      }
  ...
  async restore({ when, filter, sources, output, verbose }, remoteRestore) {
    ...
    for (let i = 0; i < lines.length; i++) {
      const entry = lines[i];
      switch(entry.type) {
        case 'HEADER': case 'STATUS': case 'COMMENT':
          break;
        case 'SOURCE':
          root = entry.root;
          ...
          break;
        default:
          if (!remoteRestore && !root) throw new Error('missing root (corrupt instance?) use --output to override');
          if (!search.ignores(entry.path)) {
            if (remoteRestore) {
              ...
              if (doRestore) {
                try {
                  await BackupInstance.restoreEntry({ from, entry, to: file, verbose });
                } catch(e) {
                  console.error('ERROR:', file, e.message, JSON.stringify(entry));
                }
              }
            }
          }
  ...
  static async createPath(path, mode = 0o755) {
    try {
      await fs.access(path, fs.W_OK);   // path exists and is writable, nothing to do other than set the mode
      await fs.chmod(path, mode);
    } catch(e) {
      await fs.mkdirp(path, mode);
    }
  }
  ...
  static async restoreEntry({ from, entry, to, verbose, isCompressedStream }) {
    ...
    switch(entry.type) {
      case 'D':
        ...
        break;
      case 'F':
        // re-create file (and path to file if necessary)
        if (verbose) console.log(to);
        await this.createPath(path.dirname(to), 0o755);
