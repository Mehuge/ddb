...
class BackupInstance {
  ...
  async createNewInstance() {
    ...
    this._lastBackup = await this._loadLastBackupHashes();
  }

  async _loadLastBackupHashes() {
    try {
      const cache = new LogParser();
      cache.parseToHashMap(await this._log.getLinesFromLog('current'));
      return cache;
    } catch(e) {
  ...
  async put(file, size, hash) {
    const lastBackup = this._lastBackup;
    if (lastBackup) {
      const entry = lastBackup.getHash(hash);
      if (entry) {
        const { variant } = entry;
        return { variant: parseInt(variant), stored: false };
      }
    }
    const bfs = this.target.fs();
