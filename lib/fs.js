...
async function readline(path) {
  ...
  return require('readline').createInterface({ input });
}
...
async function signature(source, opts) {
  ...
  opts.signature = (index, offset, size, sum) => { signature.blocks.push({ sum, size }); };
  signature.hash = await hash(reader, opts);
  signature.size = opts.size|0;
  if (signature.size != (signature.blocks.length - 1) * opts.blockSize + signature.blocks[signature.blocks.length - 1].size) {
    throw new Error('invalid signature generated, size does not match');
  }
  return signature;
}
