...
class BackupLog {
  ...
  getLogName(when) {
    return path.join(this.root, 'backups', `${this.name}.${when}`);
  }

  async create(when) {
    this.log = await fs.open(this.getLogName(when), 'w', 0o666);
  ...
  async writeEntry({ type, mode, uid, gid, ctime, mtime, size, hash, variant, fn }) {
    await this.log.appendFile(
      `${type} ${uid}:${gid}:${mode.toString(8)} ${ctime.toISOString()} ${mtime.toISOString()} -`
      + ` ${size|0} ${hash} ${variant} ${JSON.stringify(fn.replace(/\\/g,'/'))}\n`
    );
  }
