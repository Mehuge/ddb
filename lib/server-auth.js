...
class Auth {
  ...
  async process({ parts, key, request, response }) {
    ...
    switch(parts.shift()) {
      case 'login':
        ...
        const secret = `${at.toISOString()}:${key}:${address}`;
        sha1.update(secret);
  ...
  authenticate({ address, token }) {
    ...
    if (token) {
      if (!token.addresses.includes(address)) {
        token.addresses.push(address);
        console.log('New IP address recorded for login', token.login, address);
      }
      token.touch();
      return token;
    }
