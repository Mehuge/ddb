...
class BackupLog {
  ...
  async create(when, comment = null) {
    ...
    await this.log.appendFile(`V${VERSION} type mode ctime mtime - size hash variant path\n`);
    if (comment) await this.writeCommentEntry(comment);
  }

  async writeCommentEntry({ comment }) {
    await this.log.appendFile(`# ${comment}\n`);
  }
  ...
  async writeEntry(entry) {
    switch(entry.type) {
      case 'SOURCE':
        await this.writeSourceEntry(entry);
        break;
      case 'COMMENT':
        await this.writeCommentEntry(entry);
        break;
      default:
  ...
  getHashesFromInstanceLog(when, hashes) {
    return new Promise((resolve, reject) => {
      const readline = fs.readline(this.getLogName(BackupLog.parseWhen(when)));
      readline.on('line', line => {
  ...
  static parse(line) {
    ...
    switch(words[0]) {
    case '#': return { type: 'COMMENT', comment: line.substr(2) };
    case 'V1': case 'V2':
      switch(words[1]) {
        case 'STATUS':
  ...
  async complete(ts = new Date()) {
    ...
    try {
      ...
      if (e.code == "EISDIR" || e.code == "ENOTSUP") {
        // EISDIR: get this on FAT32 filesystem where links are not supported
        // so copy instead.
        // ENOTSUP: get this on a network share
