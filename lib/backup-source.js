...
class BackupSource {
	...
	async backupTo(destination, stats, log) {
		log.writeSourceEntry({ root: this.src });
		await this._backupDir(this.src, destination, stats, log);
	}

	async _log(log, type, fn, stats, hash = '-', variant = 0) {
		if (this.verbose) console.log(path.join(this.src, fn));
		const { mode, ctime, mtime, atime, size } = stats;
		await log.writeEntry({ type, mode, ctime, mtime, atime, size, hash, variant, fn });
	}
	...
	async _backupDir(dirname, destination, stats, log) {
		try {
			const fstat = await fs.stat(dirname);
			await this._log(log, 'D', dirname.substr(this.src.length+1), fstat);
		} catch(e) {
			if (e.code == 'ENOENT') {
		...
		for (let i = 0; i < dir.length; i++) {
			...
			if (type) {
				...
				if (!this._checkExcluded(fn.substr(l))) {
					switch(type) {
					case 'D':
						await this._backupDir(fn, destination, stats, log);
						break;
					case 'F':
						await this._backupFile(fn, destination, stats, log);
						break;
					default:
						// ignore other types
						break;
	...
	async _backupFile(fn, destination, stats, log) {
		try {
			...
			await this._log(log, 'F', fn.substr(this.src.length+1), fstat, hash, variant);
			if (stored) {
