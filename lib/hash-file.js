...
function createHashPassThrough(opts = {}) {
  ...
  const stream = new Transform({
    ...
    flush(callback) {
      hashResolve(sum.digest(encoding));
      callback();
    }
  });
  stream.on('error', hashReject);
  return { stream, hash: hashPromise };
}

const hashFileImpl = function (source, opts = {}) {
  const encoding = opts.encoding || 'hex';
...
module.exports = hashFileImpl;
module.exports.createHashPassThrough = createHashPassThrough;
