...
class LocalBackup {
      ...
      if (key) {
        ...
        if (!(key in hashes)) {
          ...
          try {
            ...
            stats.damaged ++;
            spinner.stop();
            console.error('ERROR ' + key, e.code, e.message);
            spinner.start();
          }
        }
      }
  ...
  async rm({ setname, when, userid, verbose, dryRun, yes }, args) {
    const spinner = await this.ora({ text: 'get logs ...', discardStdin: !!yes });
    spinner.start();
    ...
    try {
      for await (const log of logs) {
        ...
        if (dryRun) {
          spinner.stop();
          console.log('DRY RUN: would remove the entries above from', log.name);
          spinner.start();
          continue;
        }
