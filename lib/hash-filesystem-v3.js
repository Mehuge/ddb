...
class HashFileSystemV3 {
	...
	_hash2name(hash, variant = 0) {
		const name = path.join(`${crc.crc8(hash.substr(0,20))}`, `${crc.crc8(hash.substr(20))}`);
	...
	keyFromFile(dir, name) {
		return path.join(dir, path.basename(name, path.extname(name))).replace(new RegExp('\\' + path.sep, 'g'),'');
	}
