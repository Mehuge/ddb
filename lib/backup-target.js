...
class BackupTarget {
  ...
  async _connectRemote() {
    const { protocol, hostname, port } = URL.parse(this.destination);
    this._target = new RemoteBackup({ protocol, hostname, port });
  }
  ...
  async getLogs(setname, instance) {
    return await this._target.getLogs(setname, instance);
  }
