...
const BackupFileSystemV5 = require('./hash-filesystem-v5');
...
class RemoteInstance {
  ...
  async getLastBackup() {
    ...
    if (res.statusCode == 200) {
      ...
      try {
        ...
        for (let key in H) {
          H[key].ctime = new Date(H[key].ctime);
          H[key].mtime = new Date(H[key].mtime);
        }
        // Normalise paths.
        const lastBackup = {};
  ...
  async put(fn, size, hash) {
    ...
    if (res.statusCode == 404) {
      let { res } = await this._putFile(fn, size, hash);
      if (res.statusCode == 200) {
        stored = true;
      }
    }
          ...
          case 'F':
            ...
            try {
              ...
              if (stat.size == entry.size) {
                const localHash = await BackupFileSystemV5.hashFile(file);  // backup server uses V5 (same as V4 so should work with V4 server)
