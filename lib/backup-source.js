...
class BackupSource {
  constructor({ src, filters, subdirs, deepscan, verbose }) {
    this.src = src;
  ...
  async backupTo(target, instance, stats) {
    instance.log().writeSourceEntry({ root: this.src });
    const filter = new Filter({ filters: this.filters });
    const subdirs = this.subdirs;
    if (subdirs.length) {
      for (const subdir of subdirs) {
        await this._backupDir(path.join(this.src, subdir), target, instance, stats, filter);
      }
    } else {
      await this._backupDir(this.src, target, instance, stats, filter);
    }
  }
  ...
  async _log(instance, type, fn, stats, hash = '-', variant = 0) {
    ...
    await instance.log().writeEntry({ type, uid, gid, mode, ctime, mtime, size, hash, variant, fn });
  }

  async _scanDir(dirname, target, instance, stats, filter) {
    ...
    for (let i = 0; i < dir.length; i++) {
      ...
      if (entry.isDirectory()) {
        ...
        if (ignored) {
          await this._scanDir(fn, target, instance, stats, filter);
        } else {
          await this._backupDir(fn, target, instance, stats, filter);
        }
      }
    }
  }

  async _backupDir(dirname, target, instance, stats, filter) {
    try {
    ...
    for (let i = 0; i < dir.length; i++) {
      ...
      if (type) {
        ...
        case 'D':
          ...
          break;
        case 'F':
          if (!ignored) await this._backupFile(fn, target, instance, stats);
          break;
        default:
          // ignore other types
          break;
  ...
  async _backupFile(fn, target, instance, stats) {
    try {
