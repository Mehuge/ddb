...
class BackupFileSystem {
  ...
  async setLocation(root, fstype = 'hash-v5') {
    this.root = root;
    switch(fstype) {
      case 'hash-v5':     /* simpler folder structure, compressed */
        this.hfs = new (require('./hash-filesystem-v5'))({ root });
        this.fstype = fstype;
        break;
  ...
  async put(file, size, hash, options = { variant: 0 }) {
    ...
    size = ''+size;
    const hfs = this.hfs;
    const key = hfs.getKey(hash, options.variant, size);
    const alreadyExists = this.fast && this._cache && this._cache[key.hash];
    ...
    if (exists) {
      ...
      await hfs.store(file, key, options.isAlreadyCompressed);
      stored = true;
    }
    if (this._cache) this._cache[key.hash] = true;
    return { variant: options.variant, stored };
  }

  async verify(size, hash, variant, compareWith) {
    const hfs = this.hfs;
    const key = hfs.getKey(hash, variant, size);
    const exists = await hfs.exists(key);
    if (!exists) throw new Error('entry not found');
    const hash2 = await hfs.hashKey(key);
    if (hash2 != hash) {
      const err = new Error(`files.db entry corrupt for ${key.name}.${size}`);
      err.code = 'ECORRUPT';
      throw err;
    }
  ...
  async restore(size, hash, variant, copyTo, asCompressedStream) {
    await this.hfs.restore(this.hfs.getKey(hash, variant, size), copyTo, asCompressedStream);
  }
  ...
  async cat(entry) {
    const key = this.hfs.getKey(entry.hash, entry.variant, entry.size);
    await this.hfs.cat(key);
  }
}
