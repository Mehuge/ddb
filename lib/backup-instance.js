...
class BackupInstance {
  ...
  async getLogEntries(when = 'current') {
    let root;
    const entries = [];
    for (let entry of await this.getLinesFromInstanceLog(when)) {
      switch(entry.type) {
        case 'SOURCE':
          root = entry.root;
          entries.push(entry);
  ...
  async getSignature(entry) {
    return await this.target.fs().getSignature(entry);
  }
