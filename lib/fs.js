...
function copy(from, to, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const { stream: hasher, hash: hashPromise } = hashFile.createHashPassThrough({ hash: 'sha256', encoding: 'hex' });
    pump(reader, hasher, writer, async (err) => {
      if (err) return reject(err);
      resolve(await hashPromise);
    });
  });
}
...
function unzip(from, to = null) {
  return new Promise((resolve, reject) => {
    ...
    handleStreamError(gunzip);
    handleStreamError(reader);
    ...
    const { stream: hasher, hash: hashPromise } = hashFile.createHashPassThrough({ hash: 'sha256', encoding: 'hex' });
    handleStreamError(hasher);
    const writer = typeof to == 'string' ? createWriteStream(to) : to;
    handleStreamError(writer);
    const task = unzipper.pipe(hasher).pipe(writer);
    task.on('finish', async err => {
      if (err) reject(err);
      else resolve(await hashPromise);
    });
  });
}
