...
class BackupSource {
  ...
  async backupTo(destination, instance, stats) {
    ...
    if (subdirs.length) {
      for (const subdir of subdirs) {
        await this._backupDir(path.join(this.src, subdir), destination, instance, stats, filter);
      }
    } else {
      await this._backupDir(this.src, destination, instance, stats, filter);
    }
  }
  ...
  async _scanDir(dirname, destination, instance, stats, filter) {
    ...
    for (let i = 0; i < dir.length; i++) {
      ...
      if (entry.isDirectory()) {
        ...
        if (ignored) {
          await this._scanDir(fn, destination, instance, stats, filter);
        } else {
          await this._backupDir(fn, destination, instance, stats, filter);
        }
      }
    }
  }

  async _backupDir(dirname, destination, instance, stats, filter) {
    try {
    ...
    for (let i = 0; i < dir.length; i++) {
      ...
      if (type) {
        ...
        case 'D':
          ...
          break;
        case 'F':
          if (!ignored) await this._backupFile(fn, destination, instance, stats);
          break;
        default:
          // ignore other types
          break;
