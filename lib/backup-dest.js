...
class BackupDest {
	...
	async complete(name, ts = new Date()) {
		const from = this.getBackupLogName(name, 'running');
		const to = this.getBackupLogName(name, ts.toISOString().replace(/[\-:.]/g,''));
		const current = this.getBackupLogName(name, 'current');
	...
	async verify(name, opts = {}) {
		return new Promise((resolve, reject) => {
			let root;
			const lines = [];
			const readline = fs.readline(this.getBackupLogName(name, opts.variant));
	...
	async getHashesFromLogFile(path, hashes) {
		return new Promise((resolve, reject) => {
	...
	async getLogs(setname = null) {
		const logs = [];
		for (const log of await fs.readdir(this.backups, { withFileTypes: true })) {
			if (log.isFile()) {
				if (!setname || setname == path.basename(log.name, path.extname(log.name))) {
					logs.push(log);
				}
			}
		}
	...
	async getActiveHashes() {
		...
		for (const log of await this.getLogs()) {
			...
			case '.running':
				...
				break;
			default:
				await this.getHashesFromLogFile(path.join(this.backups, log.name), hashes);
				break;
			}
		}
		return hashes;
	...
	async clean(opts = {}) {
		const hashes = await this.getActiveHashes();
