...
const finishedAsync = require('util').promisify(finished);
...
class RemoteBackup {
  constructor({ protocol, hostname, host, port, verbose }) {
	...
	switch(protocol) {
	case 'http:': this.http = require('http'); break;
	case 'https:': this.http = require('https'); break;
	}
    this.token == null;
    ...
    return new Promise((resolve, reject) => {
      ...
      const request = this.http.request(options, res => {
        if (onbody) {
          ...
          const onbodyPromise = onbody(reader);
          res.on('error', (err) => reader.destroy(err));
          res.pipe(reader);
          (async () => {
            try {
              await Promise.all([finishedAsync(reader), onbodyPromise]);
              return resolve({ res });
            } catch(err) {
              reader.destroy(err);
              return reject(err);
