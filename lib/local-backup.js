...
class LocalBackup {
  ...
  async fsck({ verbose }) {
    ...
    memtrack();
    let spinner = ora({ text: 'loading hashes ...', discardStdin: true }).start();
    let hashes;
    try {
  ...
  async rm({ setname, when, userid }, args) {
    const search = new Filter({ filters: [ "-**", ...args.map(arg => `+**/${arg}`) ]});
    ...
    for await (const log of logs) {
      ...
      const { userid } = log;
      const files = await this.getFiles(setname, when, userid);
      const remove = {};
      for (let entry of files) {
        switch(entry.type) {
        case 'F':
          if (!search.ignores(entry.path)) {
            console.log(`REMOVE ${setname}.${when} ${entry.path}`);
            remove[entry.path] = true;
          }
          break;
        }
      }
      const filtered = files.filter(file => !(file.type == 'F' && remove[file.path]));
