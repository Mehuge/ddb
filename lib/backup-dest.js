...
class BackupDest {
	...
	constructor({ destination, filesystem, create }) {
	...
	async initConfig() {
		this.config = { version: VERSION, fstype: 'hash-v1' };
	...
	async init(create) {
		...
		this.filesystem.setLocation(this.filesDb, this.config.fstype);
	}
	...
	async _removeObsolete(root, hashes) {
		...
		for (let i = 0; i < dir.length; i++) {
			...
			if (entry.isDirectory()) {
				...
				const key = root.substr(s).replace(new RegExp('\\' + path.sep, 'g'),'');
				if (!(key in hashes)) {
